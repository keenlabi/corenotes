import styles from "./extendedassessmentsessionmodal.module.css";
...
export default function ExtendedAssessmentSessionModal({
	assessmentSessionId,
	closeModal,
}: {
	...
	const [completeAssessmentState, setCompleteAssessmentState] =
		useState(individualState);

	const assessmentSessionResponse = useFetchIndividualAssessmentSession(
	...
	function selectAnswer(value: string, questionId: string) {
		const assessmentQuestions = [
			...individualState.assessments.session.questions,
		];
		const questionIndex = assessmentQuestions.findIndex(
	...
	function checkAssessmentCompletion() {
		...
		if (
			answeredQuestions ===
			individualState.assessments.session?.questions.length
		) {
				...
				.catch((error) => {
					...
					setIndividualState((state) => {
						return {
							...
							error: true,
							message:
								error.message ?? "There was an error saving session, try again",
						};
					});
				});
		}
				...
				<div className={styles.header}>
					<div className={styles.heading}>
						<div className={styles.title}>
							{individualState.assessments.session.title}
						</div>
						<div className={styles.status}>
							{individualState.assessments.session.status}
						</div>
					</div>
					<IconCancelCircle
						onClick={() =>
