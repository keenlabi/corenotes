import styles from "./addnewbehaviormodal.module.css";
...
import { addBehaviorToIndividualAction } from "src/features/Individual/action";
import { ISetTagsInputFormData, ITagsInputFormData } from "src/components/FormComponents/InputField/TagInputField/types";

export default function AddNewBehaviorModal({ closeModal }:{ closeModal:()=> void }) {

    const params = useParams();

    const [individualState, setIndividualState] = useIndividualState();

    const [behaviorModel, setBehaviorModel] = useState<formFieldType>({
        type: "text",
        label: "Description",
        value:"",
        error: "",
    ...
    const [goalsModel, setGoalsModel] = useState<ITagsInputFormData>({
        label: "Goals",
        name: "behavior-goals",
        value: [],
        error: "",
    ...
    function saveInputArray(value:string[], model:ITagsInputFormData, setModel:ISetTagsInputFormData) {
        model.value = value;
        validateModel(model);
        setModel({...model});
    ...
    function validateForm() {
        ...
        if(!behaviorModel.validated) {
            setIsFormValid(false)
            return false;
        }
        
        if(!goalsModel.validated) {
            setIsFormValid(false)
            return false;
        }
    ...
    function submitBehaviorService() {
        if(validateForm()) {
            const payload = {
                description: behaviorModel.value!,
                goals: goalsModel.value,
                schedule: {
            ...
            .then((response)=> {
                setIndividualState(state => ({
                    ...
                    error: false,
                    behaviorsServices: response.data
                }))
        ...
        <ModalContainer close={closeModal}>
            <div className={styles.add_new_goal_modal}>
                <div className={styles.header}>
                    <div className={styles.title}>Set New Behavior</div>
                ...
                <div className={styles.body}>
                    <div className={styles.section_header}>
                        ...
                        <div className={styles.text}>Behavior details</div>
                    </div>

                    <RowContainer alignment="top">
                        <InputField
                            label={behaviorModel.label}
                            type={behaviorModel.type}
                            error={behaviorModel.error}
                            onInput={(value:string)=> setInput(value, behaviorModel, setBehaviorModel)}
                        />

                        <TagInputField 
                            label={goalsModel.label!}
                            placeholder={""}
                            value={[]}
                            error={goalsModel.error}
                            onTagAdded={(value:string[])=> saveInputArray(value, goalsModel, setGoalsModel)}
                        />
                    </RowContainer>
